...
import { LayoutDashboard, FileText, Users, MessageSquare, HelpCircle } from 'lucide-react';
  ...
  return (
    <div className="w-64 bg-[#1e293b] min-h-screen flex flex-col text-gray-300 fixed left-0 top-0 z-50 font-sans">
      ...
      <nav className="flex-1 px-4 space-y-1">
        ...
        <NavItem icon={<FileText size={20} />} label="Application List" />
        <NavItem icon={<Users size={20} />} label="Staff Management" />
        {/* Mục đang Active */}
        <NavItem icon={<MessageSquare size={20} />} label="Feedback List" active />
      </nav>
...
const NavItem = ({ icon, label, active }) => (
  <Link to="#" className={`flex items-center gap-3 px-4 py-3 rounded-lg cursor-pointer transition-all duration-200 ${
    active 
